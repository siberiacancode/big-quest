...
import { ActivityMedia } from '../ActivityDialog/components/ActivityMedia/ActivityMedia';
...
interface ActionActivityFormProps<ActionType extends Exclude<ActivityActionType, 'info'>> {
  onAction: () => void;
...
export const ActionActivityForm = <ActionType extends Exclude<ActivityActionType, 'info'>>({
  onAction,
  onEdit = () => {},
  activity,
  actionType,
  ...
  return (
    <div className='flex h-full flex-col justify-between gap-4 overflow-y-auto smx:px-0'>
      <div className='flex h-[418px] gap-6 2smx:h-[600px]'>
        {(state.isLoading || state.isPostActivityLoading) && <ActivityMediaSkeleton />}
        {!state.isLoading && !state.isPostActivityLoading && (
          <ActivityMedia
      ...
      <Form {...form}>
        <form onSubmit={functions.onSubmit} className='flex h-full flex-col justify-between gap-4'>
          <fieldset disabled={state.isLoading} className='flex w-full flex-col items-end'>
            {(state.isLoading || state.isPostActivityLoading) && <ActivityActionFormSkeleton />}
            {!state.isLoading && !state.isPostActivityLoading && (
                ...
                <div className='flex justify-between gap-24 smx:flex-col smx:gap-2'>
                  <div className='flex-1 space-y-3'>
                    <FormField
                      ...
                      name='name'
                      disabled={state.isPostActivityLoading}
                      render={({ field }) => (
                        <FormItem>
            ...
            <div className='flex w-full justify-center'>
              <Button
                ...
                variant='secondary'
                disabled={state.isLoading || state.isPostActivityLoading}
              >
