...
import PartnerProfileChangesLoading from '../../loading';

import { usePartnerProfileChangesForm } from './hooks/usePartnerProfileChangesForm';

interface PartnerProfileChangesFormProps {
  organizationId: string;
}

export const PartnerProfileChangesForm = ({ organizationId }: PartnerProfileChangesFormProps) => {
  const i18n = useI18n();
  const { form, functions, state } = usePartnerProfileChangesForm({ organizationId });

  if (state.query.isLoading) return <PartnerProfileChangesLoading />;
